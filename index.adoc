...
==== SysTick
https://github.com/JeremyGrosser/rp2040_hal/blob/master/src/drivers/rp-systick.ads[RP.SysTick] implements the https://github.com/Fabien-Chouteau/hal/blob/master/src/hal-gpio.ads[HAL.Time] interface using the standard 24-bit SysTick timer included with each ARM Cortex-M0+ core. SysTick is configured to generate an interrupt every 1 millisecond (1 KHz). `Delay_Microseconds` is not supported with this driver. Interrupts are used to implement all of the delay procedures. The interrupt handler increments a 64 bit variable. `RP.SysTick.Clock` returns the current value of that variable, rather than the hardware counter.
