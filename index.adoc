...
There is a port of Ravenscar for the RP2040 in the bb-runtimes repository, but it has not yet been merged and is not distributed with the GNAT ARM toolchain. However, projects can still use the Ravenscar runtime by depending on the https://github.com/JeremyGrosser/ravenscar_full_rp2040[ravenscar_full_rp2040] Alire crate. This crate is not included in the Alire index as it is expected to be deprecated once RP2040 support is merged into bb-runtimes.
