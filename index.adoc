...
Until those changes are made, the https://github.com/JeremyGrosser/pico_examples/blob/master/feather_rp2040_bsp/alire.toml[feather_rp2040_bsp] example shows how a BSP project can select boot2 code for a different flash chip. This example uses the `generic_qspi` code, which should be compatible with most SPI flash chips but will not take advantage of performance enhancing features.
