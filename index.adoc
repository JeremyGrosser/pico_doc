...
===== Low level interface
The https://github.com/JeremyGrosser/rp2040_hal/blob/master/src/drivers/rp-i2c.ads[RP.I2C] package implements a lower level interface, supporting both controller (master) and target (slave) modes. The following example writes a register address to a target device, then reads a single byte value. The `Stop` argument to `Start_Write` ensures that the controller does not release the bus between the write and read and instead performs a repeated start.
...
See https://github.com/JeremyGrosser/pico_examples/blob/master/i2c_demo/src/i2c_demo.adb[i2c_demo] for a more complete example.
