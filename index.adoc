...
image::pico.jpg[Raspberry Pi Pico,width=300,role="related right",title="Raspberry Pi Pico",caption="",link="/img/pico.jpg"]

This project provides libraries and xref:examples[examples] for the https://en.wikipedia.org/wiki/Ada_(programming_language)[Ada programming language] on the https://www.raspberrypi.org/documentation/microcontrollers/rp2040.html[RP2040] microcontroller and the https://www.raspberrypi.org/products/raspberry-pi-pico/[Raspberry Pi Pico] development board.
...
If Alire prompts you to install tools like unzip and git, accept these changes. When prompted to select a toolchain, choose the most recent versions of `gnat_arm_elf` and `gprbuild`.
...
- https://github.com/JeremyGrosser/pico_bsp[pico_bsp] provides pin definitions, boot code, and drivers for the Raspberry Pi Pico development board and compatible addons.
...
----
Some oscillators may need a longer startup delay. If your system clock doesn't run at the frequency you expect, try adjusting this. NXP's https://www.nxp.com/docs/en/application-note/AN3208.pdf[Crystal Oscillator Troubleshooting Guide] has some good advice for troubleshooting this if you're building a custom board.
...
The `RP.Clock.Frequency` function uses the internal frequency counter to measure internal clocks. This is used by to configure dividers and enforce minimum operating frequencies in preconditions. Most drivers provide convenience wrappers that perform these calculations for you.
[source,ada]
----
declare
...
The RP2040's ROM includes a set of floating point functions using the https://en.wikipedia.org/wiki/CORDIC[CORDIC algorithm], implemented in assembly. The Cortex-M0+ has no hardware floating point unit, so if you need to do floating point math, this is as fast as it's gonna get. GCC will use it's own math library by default, which will generally be slower as fetching code from flash is slower than ROM access.
...
==== Boot code
As the RP2040 has no onboard flash, code is executed from an external flash chip. The `XIP_SSI` (Execute In Place Slave Serial Interface) peripheral needs to be configured for the flash chip in use. At power on, the RP2040's ROM uses a slow `0x04` SPI flash read command to load the first 256 bytes of flash into SRAM, verify a checksum at the end of that page, then jumps to it and begins executing. This section is known as `.boot2` in linker scripts. boot2 should disable the SSI, reconfigure it for the specific flash chip on the board, then re-enable it and jump to the beginning of user code, now executing from the memory mapped flash chip.

Currently, pico_bsp contains .boot2 code copied from pico-sdk and pre-compiled. This is not ideal, both from a licensing and flexibility perspective. Signficant progress has been made in rewriting .boot2 in Ada and we hope to migrate to this in the near future. For up to date information on the progress of these efforts, xref:community_support[join us on Gitter.im].

Until those changes are made, the https://github.com/JeremyGrosser/pico_examples/blob/master/adafruit_feather_rp2040/blink_feather.gpr#L16[adafruit_feather_rp2040] example shows how a BSP project can select boot2 code for a different flash chip. This example uses the `generic_03h` code, which should be compatible with most SPI flash chips but will not take advantage of performance enhancing features.

==== Linker script
Some startup code and a linker script are needed to generate an executable binary for the RP2040. The Ravenscar runtimes include these and will compile them along with your code. If you are using a ZFP runtime, pico_bsp provides reasonable defaults in the https://github.com/JeremyGrosser/pico_bsp/blob/master/zfp_startup[zfp_startup] directory. If you wish to customize these, copy zfp_startup to your BSP crate and modify the linker options in your .gpr to point to the customized linker script and crt0.S.
...
The https://github.com/JeremyGrosser/pico_examples/blob/master/adafruit_feather_rp2040/src/adafruit_feather_rp2040.ads[Adafruit Feather RP2040 example] provides pin definitions for a custom board.
